...
newtype CRC16 = CRC16 { crc16 :: Word16 } deriving (Show, Eq, Ord)


instance CRC CRC16 where
  type CRCWord CRC16 = Word16
  initCRC = CRC16 0
  crcWord = crc16
  updateDigest = updateDigest16
  crcTable = table


-------------------------------------------------------------------------------
updateDigest16 :: CRC16 -> ByteString -> CRC16
updateDigest16 crc = BS.foldl go crc
  where
    go (CRC16 crc') b8 =
      let idx = fromIntegral (crc' `xor` b16) .&. 0xff
          b16 = fromIntegral b8
      in CRC16 ((table ! idx) `xor` ((crc' `shiftR` 8) .&. 0x00ffff))
